...
from rjmcmc.process import PostProcess
from rjmcmc.util import expand_array_columns, organize_results
...
class InversionHandler:
    ...
    def save_states(self) -> None:
        """Save parameter states to an attribute and as Pickle."""
    ...
    def save_samples(self, concat_samples: bool, use_multiindex: bool | None) -> None:
        """Save posterior samples to an attribute and as Parquet and CSV.
        ...
        use_multiindex : bool | None, optional
            ...
            See more details in rjmcmc.util.expand_array_columns().
            If None, not saved as CSV.
        ...
        # Save only the current posterior samples
        else:
    ...
    def load_samples(self, load_path: Path | str) -> None:
        "Load posteriors samples from Parquet."
        self.postsamples = pd.read_parquet(load_path)
    ...
    def process(
        ...
        None
        """
        self.save_samples(concat_samples, use_multiindex)
